...
class VoxelNet(nn.Module):
    ...
    @torch.no_grad()
    def voxelize(self, points, ranges):
        coors = []
        # dynamic voxelization only provide a coors mapping
        for i, res in enumerate(points):
            res_coors = self.voxel_layer(res, ranges[i])
    ...
    def forward(self, x, r):
        feats, coords = self.voxelize(x, r)
        voxel_features, coords = self.voxel_encoder(feats, coords, r)
...
class DynamicSimpleVFE(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, features, coors, range, *args, **kwargs):
        """Forward function.
        ...
        # grid_shape = torch.tensor([128, 128, 20], dtype=torch.float32, device=range.device)
        # voxel_size = (range[3:] - range[:3]) / grid_shape
        # voxel_size = voxel_size.tolist()
        # point_cloud_range = range.tolist()

...
class TrainSampler(torch.utils.data.Dataset):
    ...
    def processing(data, config):
        ...
        range_this = [-base_box.wlh[1], -base_box.wlh[0], -base_box.wlh[2] * 0.75,
                      base_box.wlh[1], base_box.wlh[0], base_box.wlh[2] * 0.75]
        this_frame_pc = points_utils.crop_pc_in_range(this_pc, base_box, range_this)
        ...
        inputs = {'prev_points': torch.as_tensor(prev_points, dtype=torch.float32),
                  ...
                  'prev_range': torch.as_tensor(range_prev, dtype=torch.float32),
                  'this_range': torch.as_tensor(range_this, dtype=torch.float32)}
        data_samples = {
